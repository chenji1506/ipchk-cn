...
useHead({
  title: 'IPv6网站检测工具 | IPv6访问支持检查 | ipchk.cn',
  meta: [
...
<template>
  <div class="title">
    <header>
      <h1>{{ $t('IPv6网站检测') }}</h1>
      <p>{{ $t('检查网站是否开启 IPv6 访问，致力于普及IPv6') }}</p>
    </header>
  </div>
  <div class="content">
    <div class="one-line">
    ...
    <div v-if="result && result.ipv4" class="result-section">
      <table class="result-table">
        ...
        <tbody>
          ...
          <tr>
            <td class="table-label">{{ $t('HTTP 状态码') }}</td>
          ...
          <tr>
            <td class="table-label">{{ $t('HTTPS 状态码') }}</td>
            <td class="table-value">
          ...
          <tr>
            <td class="table-label">{{ $t('DNS 查询耗时') }}</td>
          ...
          <tr>
            <td class="table-label">{{ $t('TCP 连接耗时') }}</td>
          ...
          <tr>
            <td class="table-label">{{ $t('HTTP 连接耗时') }}</td>
          ...
          <tr>
            <td class="table-label">{{ $t('首字节耗时') }}</td>
          ...
          <tr>
            <td class="table-label">{{ $t('页面大小') }}</td>
    ...
    <div v-if="result && result.ipv4  && result.ipv6&& result.ipv4.is_reachable && result.ipv6.is_reachable">
      <h3>{{ $t('结论：') }}<el-icon><CircleCheckFilled style="color: lightgreen;"/></el-icon>网站{{ extractHost(testDomain) }} 支持IPv6访问 </h3>
      <p><el-icon><InfoFilled style="color: lightgreen;"/></el-icon>{{ $t('请把下方代码贴到网站底部，把这个好消息告诉你的用户，以便用户核验。') }}</p>
        <img src="/ipv6-s1.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s1.svg"&gt;&lt;/a&gt;</code></pre>
        <img src="/ipv6-s2.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s2.svg"&gt;&lt;/a&gt;</code></pre>
        <img src="/ipv6-s3.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s3.svg"&gt;&lt;/a&gt;</code></pre>
        <img src="/ipv6-s4.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s4.svg"&gt;&lt;/a&gt;</code></pre>
        <img src="/ipv6-s5.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s5.svg"&gt;&lt;/a&gt;</code></pre>
        <img src="/ipv6-s6.svg"/>
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-s6.svg"&gt;&lt;/a&gt;</code></pre>
        <div class="one-line">
        ...
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-certified-lite-s1.svg"&gt;&lt;/a&gt;</code></pre>
        <div class="one-line">
        ...
        <pre><code>&lt;a href="{{ config.siteUrl }}ipv6webcheck/?site={{ extractHost(testDomain) }}" :title="$t('本站支持 IPv6访问')" target='_blank'&gt;&lt;img style='display:inline-block;vertical-align:middle' :alt="$t('本站支持 IPv6访问')" src="https://ipchk.cn/ipv6-certified-s1.svg"&gt;&lt;/a&gt;</code></pre>
        <p>{{ $t('提示：修改IPv6徽标文件名，可修改对应样式') }}</p>
    </div>
    <div v-else-if="result && result.ipv4 && result.ipv6 && result.ipv4.is_reachable && !result.ipv6.is_reachable">
      <h3>{{ $t('结论：') }}<CircleCloseFilled style="width: 1.3em;color: red;"/>网站{{ extractHost(testDomain) }} 不支持IPv6访问 </h3>
      <h2>{{ $t('国家正在支持IPv6发展，我建议你赶紧想办法给IPv6适配') }}</h2>
      <el-image src="/jingya.jpg"></el-image>
    </div>
    <div v-else-if="result && (!result.ipv6?.is_reachable && !result.ipv4?.is_reachable)">
      <h3>{{ $t('结论：') }}<CircleCloseFilled style="width: 1.3em;color: red;"/>网站{{ extractHost(testDomain) }} 不可达 </h3>
      <h2>...</h2>
      <el-image src="/jingya.jpg"></el-image>
    </div>
    <blockquote>
      {{ $t('网站不支持 IPv6可能原因：') }}<br/>
      <br/>
      {{ $t('1. 网站所在服务器未开启 IPv6，请参考 网站开启 IPv6 的三种方式') }}<br/>
      {{ $t('2. 网站所在服务器已开启 IPv6，但防火墙未对源地址是 IPv6 地址(::/0)的 443（HTTPS）端口开放访问') }}<br/>
      {{ $t('3. 网站所在服务器已开启 IPv6，但未开启SSL证书，请参考 Nginx 开启 IPv6 SSL') }}<br/>
    </blockquote>
